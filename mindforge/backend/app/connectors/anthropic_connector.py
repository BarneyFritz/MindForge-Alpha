...
class AnthropicConnector(BaseLLMConnector):
    async def authenticate(self) -> bool:
        return bool(self.config.headers.get("x-api-key"))

    async def generate_response(self, prompt: str, context: Optional[str] = None) -> LLMResponse:
        start = time.perf_counter()
        content = f"[Claude:{self.config.model_name}] Response to: {prompt[:100]}"
        return LLMResponse(content=content, model_used=self.config.model_name or "claude-3-opus", response_time=time.perf_counter()-start)
